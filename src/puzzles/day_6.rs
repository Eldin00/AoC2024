...
fn part_2(mut map: Vec<Vec<bool>>, start: &(usize, usize), visited: HashMap<(usize, usize), u32>) {
    ...
    for i in 0..map.len() {
        for j in 0..map[0].len() {
            if map[i][j] || start == &(i, j) || !visited.contains_key(&(i, j)) {
                continue;
            }
            map[i][j] = true;
            ...
            while guard.step(&map) {
                if visited.contains_key(&guard.loc)
                    && visited.get(&guard.loc).unwrap().contains(&guard.dir)
                {
                    result += 1;
                    break;
                }
